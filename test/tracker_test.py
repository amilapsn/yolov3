...
from utils.track_utils import initiate_tracker, detect_key_points, initiate_matcher
from utils.track_utils import NewBBox, track_bboxes
...
for i, (path, img, im0, vid_cap) in enumerate(dataloader):
    ...
    new_bbox_list =[]
    print('Time taken before keypoints : (%.3fs)' % (time.time() - t))
    if detections is not None and len(detections) > 0:
        ...
        for *xyxy, conf, cls_conf, cls in detections:
            ...
            # t1 = time.time()
            kp, des = detect_key_points(im0, xyxy, tracker)
    ...
    print('Time taken before tracking : (%.3fs)' % (time.time() - t))
